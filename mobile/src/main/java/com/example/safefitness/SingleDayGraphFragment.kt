...
class SingleDayGraphFragment : Fragment() {
    ...
    private fun loadDayData(date: String) {
        CoroutineScope(Dispatchers.IO).launch {
            val rawData = if (dataType == "steps") {
                fitnessDao.getDataForCurrentDay(date).mapNotNull { it.steps?.let { steps -> it.date to steps as Number } }
            } else {
                fitnessDao.getDataForCurrentDay(date).mapNotNull { it.heartRate?.let { heartRate -> it.date to heartRate as Number } }
            }
